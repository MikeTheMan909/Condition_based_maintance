...
#define RADIO_GPIO_IRQ        SPIRIT1_GPIO0_EXTI_IRQn
#define SPI_ENTER_CRITICAL()  HAL_NVIC_DisableIRQ(RADIO_GPIO_IRQ);
#define SPI_EXIT_CRITICAL()   HAL_NVIC_EnableIRQ(RADIO_GPIO_IRQ);

#define RadioSpiCSLow()       HAL_GPIO_WritePin(SPI_Cs_GPIO_Port, SPI_Cs_Pin, GPIO_PIN_RESET)
#define RadioSpiCSHigh()      HAL_GPIO_WritePin(SPI_Cs_GPIO_Port, SPI_Cs_Pin, GPIO_PIN_SET)

#define RADIO_SPI_TIMEOUT_MAX ((uint32_t)1000)

#define RADIO_SDN_PORT        SPIRIT_SHUTDOWN_GPIO_Port
#define RADIO_SDN_PIN         SPIRIT_SHUTDOWN_Pin
